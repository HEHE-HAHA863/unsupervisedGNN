...
def test_single(gnn, gen, n_classes, it):
    ...
    adjacency_eigvecs = eigvecs_W[:, np.argsort(eigvals_W)[-n_classes:]]  # (1000, 2)
    ...
    laplacian_eigvecs = eigvecs_L[:, np.argsort(eigvals_L)[:n_classes]]  # (1000, 2)
    ...
    output_filename = "all_test_results_p=0.1_q=0.05_classes=3.xlsx"
    ...
    # --- 6. 保存结果 ---
    n = n_classes  # 比如 2 或 3
    # 构造字典动态选取前 n_featuress
    data = {}
    for i in range(n):
        data[f'GNN_Feature{i + 1}'] = penultimate_features[:, i]
    for i in range(n):
        data[f'Adj_EigVec{i + 1}'] = adjacency_eigvecs[:, i]
    for i in range(n):
        data[f'Lap_EigVec{i + 1}'] = laplacian_eigvecs[:, i]
    data['Labels'] = labels.squeeze(0).cpu().numpy()
...
if __name__ == '__main__':
    ...
    if (args.mode == 'test'):
        ...
        filename = "gnn_J4_lyr20_Ntr1000_num6000_p= 0.2_q= 0.1_ncalsses=3"
        ...
        if ((filename != '') and (os.path.exists(path_plus_name))):
            ...
            print ('No such a gnn exists; creating a brand new one')
            filename = 'gnn_J' + str(args.J) + '_lyr' + str(args.num_layers) + '_Ntr' + str(args.N_train) + '_num' + str(args.num_examples_train)+ '_p=' + str(args.p_SBM)  + '_q=' + str(args.q_SBM) + '_ncalsses=' + str(args.n_classes)
            path_plus_name = os.path.join(args.path_gnn, filename)
            if (args.generative_model == 'SBM_multiclass'):
